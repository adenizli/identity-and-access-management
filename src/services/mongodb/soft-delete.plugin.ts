...
export type SoftDeleteOptions = {
  field?: string; // defaults to 'deletedAt'
  defaultFilter?: 'exclude' | 'include' | 'only'; // exclude = normal; include = all; only = only-deleted
};
...
export function softDeletePlugin(schema: Schema) {
  ...
  function apply(q: Query<any, any>) {
    const qopts: any = q.getOptions?.() ?? {};
    ...
    const current = (q as any).getFilter?.() ?? {};
